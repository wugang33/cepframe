...
namespace cep {
    ...
    Config::Config(string filename, string delimiter,
            string comment)
    : m_Delimiter(delimiter), m_Comment(comment), if_exit(true) {
    ...
    //Config::Config()
    //: m_Delimiter(string(1, '=')), m_Comment(string(1, '#')), fl_name("./conf/config.sys") {
    //    std::ifstream in(fl_name.c_str());
    //
    //    if (!in) throw File_not_found(fl_name);
    //
    //    in >> (*this);
