    ...
    switch (info->type) {
        ...
        case 0x1001:
        {
            ...
            if (ztpserver->Start()) {
                ...
                char buf[2] = {0};
                *(unsigned short *) buf = htons_ex(port);
                ...
                char buf[2] = {0};
                *(unsigned short *) buf = htons_ex(01);
        ...
        case 0x1002:
        {
             ...
             */
            int server_nr = htons_ex(*(unsigned short *) data);
            for (int i = 0; i < server_nr; i++) {
                unsigned int node_id = htonl_ex(*(unsigned int *) ((char*) data + i * 10 + 2));
                unsigned int ip = htonl_ex(*(unsigned int *) ((char*) data + i * 10 + 2 + 4));
                string ipstr = cep::ip2str(ip);
                unsigned short port = htons_ex(*(unsigned short *) ((char*) data + i * 10 + 2 + 8));
...
void CEPDispatch::doSlaveRequest(const void*data, ZTP_DATA_INFO*info, CEPConnHandler *cephandler) //处理slave发送过来的请求   命令字 200
{
    ...
    switch (info->type) {
        case 0x0001:
        {
             ...
             */
            unsigned short port = ntohs_ex(*(unsigned short*) data);
             ...
             */
            {
                boost::mutex::scoped_lock slock(this->m_smatrix_lock_);
                ...
                if (m_smatrix->m_rowhead[csn->m_nodeid]) {
                    ...
                    while (nodeptr) {
                        ...
                        if (conn_iter != this->m_connmap.end()) {
                            ...
                            char buffer[12] = {0};
                            *(unsigned short*) (buffer) = htons_ex(1);
                            *(unsigned int*) (buffer + 2) = htonl_ex(cephandler->getRemoteId());
                            *(unsigned int *) (buffer + 6) = htonl_ex(csn->m_ip);
                            *(unsigned short*) (buffer + 10) = htons_ex(port);
        ...
        case 0x0002:
        {
             ...
             * 节点是服务器 
             */
            unsigned int node_id = ntohl_ex(*(unsigned int *) data);
            ...
            if (node_iter != m_nodemap.end()) {
                ...
                char buffer[12] = {0};
                *(unsigned short*) (buffer) = htons_ex(1);
                *(unsigned int*) (buffer + 2) = htonl_ex(node_iter->second->m_nodeid);
                *(unsigned int *) (buffer + 6) = htonl_ex(node_iter->second->m_ip);
                *(unsigned short*) (buffer + 10) = htons_ex(node_iter->second->m_port);
            ...
            if (!findserv) {
                ...
                if (node_iter != m_nodemap.end()) {
                    findserv = true;
                    *(unsigned short*) (buf) = htons_ex(1);
                    *(unsigned int*) (buf + 2) = htonl_ex(node_iter->second->m_nodeid);
                    *(unsigned int *) (buf + 6) = htonl_ex(node_iter->second->m_ip);
                    *(unsigned short*) (buf + 10) = htons_ex(node_iter->second->m_port);
                }
                this->m_nodemaplock.unlock();
                if (findserv) {//发现请求的客户端开启了服务 那么告诉对方来连接你
                    ...
                    m_smatrix_lock_.lock();
                    this->m_smatrix->insert(remoteid, node_id, 1);
                    m_smatrix_lock_.unlock();
                    char buffer[2] = {0};
                    *(unsigned short*) buffer = htons_ex(0x3389);
...
bool CEPDispatch::requestConnection(unsigned int nodeid) {
    if (this->m_masterclient->IsConnected()) {
        ...
        record_header->type = htons_ex(0x0002);
        record_header->ver_info = 0;
        record_header->format = 0;
        record_header->data_len = htonl_ex(sizeof (INT));
        *(unsigned int *) (record_header->record_data) = htonl_ex(nodeid);
...
bool CEPDispatch::sendCommand(CEPConnHandler*handler, unsigned char flag, unsigned int type, const char *buf, unsigned int buflen) {
    ...
    record_header->type = htons_ex(type);
    record_header->format = 0;
    record_header->ver_info = 0;
    record_header->data_len = htonl_ex(buflen);
...
void CEPDispatch::doConnect(const void*data, ZTP_DATA_INFO*info, CEPConnHandler * cephandler) {
    ...
    if (CEP_MASTER_NODEID == cephandler->getLocalId()) {//是master 如果这个节点是服务节点 那么通知开启服务 如果是客户端节点 那么连接到注册了的客户端
        ...
        boost::mutex::scoped_lock slock(m_smatrix_lock_);
        ...
        if (m_smatrix->m_colhead[node_id]) {
            ...
            while (nodeptr) {
                ...
                for (; node_iter != this->m_nodemap.end(); node_iter++) {
                    ...
                    *((unsigned int*) (commandvec.data() + oldsize)) = htonl_ex(node_iter->second->m_nodeid);
                    *((unsigned int*) (commandvec.data() + oldsize + 4)) = htonl_ex(node_iter->second->m_ip);
                    *((unsigned short*) (commandvec.data() + oldsize + 8)) = htons_ex(node_iter->second->m_port);
            ...
            *((unsigned short*) commandvec.data()) = htons_ex(*((unsigned short*) commandvec.data()));
